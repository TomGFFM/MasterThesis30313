...
class Hyperparameters(object):
    ...
    def get_params(trial):
        agent_hyper_params = {
            "batch_size": trial.suggest_categorical("batch_size", [32, 64, 128]),                           # size of each batch pushed through the network
            ...
            "learn_start": 10,                                                                              # number of episodes which have to be played before the training starts (10% of n_episodes)
            ...
            "punish_factor": 1.1,                                                                           # factor which decreases the reward in reward shaping
        }
        ...
        network_hyper_params = {
            ...
            "num_heads": trial.suggest_categorical("num_heads", [2, 4, 8]),                                 # number of attention heads in transformer layers
            "num_layers": trial.suggest_categorical("num_layers", [2, 4, 8]),                                   # number of transformer encoding layers
            "size_linear_layers": trial.suggest_categorical("size_linear_layers", [64, 96, 128]),         # size of the fully connect linear layers in the transformer encoder setup
