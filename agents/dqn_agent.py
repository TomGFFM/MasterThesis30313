...
class DeepQNetworkAgentPrioritizedNoisy:
    ...
    def learn(self, experiences: Tuple[
        ...
        This method mainly implements the loss calculation and optimization for training the policy network in a
        Deep Q-Network (DQN) setup. The process involves the following steps:

        1. Experience Extraction: Retrieve a batch of experiences from the replay buffer, including states, actions,
        rewards, next_states, terminations, truncations, importance sampling weights, and indices.

        2. Expected Q-Values Calculation: Compute the expected Q-values from the policy network based on the current
        states and actions.

        3. Target Q-Values Calculation: Use the target network to calculate the maximum predicted Q-values for the
        next states.

        4. Q-Target Computation: Calculate the target Q-values by adding the rewards to the discounted maximum
        Q-values from the target network, following the formula:

           Q_target = r + gamma * max_a' Q(s', a'; theta_minus)

           where:
           - Q_target is the target Q-value.
           - r is the immediate reward.
           - gamma is the discount factor for future rewards.
           - s' is the next state.
           - theta_minus represents the parameters of the target network.

        5. Initial Loss Calculation: Apply the loss function to the expected Q-values and the target Q-values to
        compute the initial loss.

        6. Loss Weighting: Adjust the loss using importance sampling weights from the replay buffer to prioritize
        significant experiences.

        7. Backpropagation: Perform backpropagation to update the policy network's parameters based on the weighted
        loss.

        The loss metric measures the discrepancy between the predicted Q-values from the policy network and the
        target Q-values from the target network. A lower loss indicates more accurate predictions of action values
        by the agent.

        In summary, the loss metric measures the discrepancy between the predicted Q-values (from the policy network)
        and the target Q-values (computed from the target network). A lower loss indicates that the agent is making
        more accurate predictions about the value of its actions.  During agent training, the above metrics are tracked,
        which allows for continuous monitoring and post-training quality evaluation. In particular, the Q-values will
        allow us to have a more detailed view on the quality of the agent's decision making and learning. These metrics,
        together with model checkpoints based on reward improvements, ensure that the training process remains effective
        and can be evaluated at various stages.

        Args:
            experiences (Tuple[torch.Tensor]): A tuple of (states, actions, rewards, next_states, terminations,
            truncations, indices) tensors.
        ...
        # According to https://github.com/vwxyzjn/cleanrl/issues/457
        # According to https://github.com/philtabor/Deep-Q-Learning-Paper-To-Code/blob/master/DDQN/ddqn_agent.py
        # According to https://github.com/udacity/deep-reinforcement-learning/blob/master/dqn/solution/dqn_agent.py
        # According to https://github.com/rlcode/per/blob/master/cartpole_per.py
        ...
        # According to https://github.com/rlcode/per/blob/master/cartpole_per.py
        # According to https://github.com/XinJingHao/Prioritized-Experience-Replay-DDQN-Pytorch/blob/main/LightPriorDQN_gym0.2x/DQN.py
